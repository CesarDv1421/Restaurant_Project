...
const ToggleAditionalButton = ({ option, id, variantSelected, setVariantSelected, setVariants, variants, price }) => {
  ...
  useEffect(() => {
    if (variants.length == 0) {
      ...
      setVariants(["Sin Agregado"]); //Agrego "Con Todo" al array
    }
  }, [variants]);
  ...
  return (
    <div>
      <label htmlFor="">
        ...
        >
          {option == "Sin Agregado" ? "Sin Agregado" : `${option} ($${price})`}
        </button>
      </label>
    </div>
  );
...
export default ToggleAditionalButton;
